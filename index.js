...
// --- CONFIGURACIÓN DE BASE DE DATOS GLOBAL ROBUSTA ---
// Usamos un bucket único con una clave más compleja para evitar colisiones
const DB_KEY = 'global_articly_v3_9922'; 
const CLOUD_URL = `https://kvdb.io/S2mP8N5Z6H7J9K1L3M4N/${DB_KEY}`;
...
// --- SERVICIO DE DATOS (REESCRITO PARA ESTABILIDAD) ---
const cloudDb = {
  fetchAll: async () => {
    try {
  ...
  sync: async (articles) => {
    try {
      // Intentamos guardar con POST y cabeceras explícitas
      ...
      if (!res.ok) {
        // Algunos buckets prefieren PUT para actualizaciones totales
        const resPut = await fetch(CLOUD_URL, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(articles),
        });
        if (!resPut.ok) throw new Error("Fallo en POST y PUT");
      }
...
const aiService = {
  generate: async (title, cat) => {
    try {
      ...
      return "No se pudo generar el contenido con IA en este momento.";
    }
  }
};
  ...
  <div className="flex flex-col items-start">
    <div className="flex items-center gap-2 bg-slate-100 px-3 py-1 rounded-full border border-slate-200">
      ...
      <span className="text-[9px] font-black text-slate-500 uppercase tracking-tighter">
        ${isSyncing ? 'Sincronizando...' : (lastError ? 'Error de Red' : 'Nube Activa')}
      </span>
    </div>
    ${lastError && html`<span className="text-[8px] text-red-400 font-bold mt-1 ml-1 uppercase">Reintentando conexión...</span>`}
  </div>
`;
...
const App = () => {
  ...
  useEffect(() => {
    loadGlobalData(true);
    const timer = setInterval(() => loadGlobalData(false), 10000);
  ...
  const handlePublish = async (newArticle) => {
    ...
    if (!success) {
      ...
      alert("La nube no respondió. El artículo se guardó localmente y se sincronizará cuando vuelva la conexión.");
    } else {
      setLastError(null);
    }
  ...
  const handleRemove = async (id) => {
    ...
    if (!success) setLastError("Delete sync failed");
    ...
    <div className="min-h-screen bg-slate-50/50">
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-slate-200 px-6 h-20 flex items-center justify-between">
        ...
        <div className="hidden md:flex flex-1 max-w-md mx-8">
          <input className="w-full bg-slate-100 border-none rounded-2xl px-6 py-3 text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all" placeholder="Buscar en la base de datos..." value=${filter.search} onChange=${e => setFilter({...filter, search: e.target.value})} />
        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        ` : html`
          <div className="py-40 text-center opacity-30">
            <p className="text-xl font-bold italic">Cargando base de datos global...</p>
          </div>
        `}
      </main>
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-2xl flex items-center justify-center p-4 sm:p-10 overflow-y-auto" onClick=${() => setSelected(null)}>
          <div className="w-full max-w-4xl bg-white rounded-[3rem] overflow-hidden shadow-2xl animate-in zoom-in-95" onClick=${e => e.stopPropagation()}>
            ...
            <div className="p-10 sm:p-16">
              <h1 className="text-4xl font-black text-slate-900 mb-6">${selected.title}</h1>
              <div className="flex items-center gap-3 mb-10 pb-8 border-b border-slate-50">
                ...
                <div>
                  <p className="text-slate-900 font-bold">${selected.author}</p>
                  <p className="text-slate-400 text-xs">Publicación Global en Articly</p>
                </div>
              </div>
              <p className="text-slate-600 text-xl leading-relaxed whitespace-pre-wrap font-serif italic">${selected.content}</p>
              <div className="mt-12 flex justify-between items-center bg-indigo-50 p-8 rounded-3xl">
                <div>
                  <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Precio Final</p>
                  <p className="text-4xl font-black text-slate-900">$${selected.price.toFixed(2)}</p>
                </div>
                <button className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black shadow-xl hover:bg-indigo-700 active:scale-95 transition-all">COMPRAR AHORA</button>
              </div>
            </div>
          </div>
        </div>
...
const ArticleForm = ({ onSave, onCancel, isSyncing }) => {
  ...
  const handleGen = async () => {
    if (!data.title) return alert("Escribe un título primero");
    setLoadingAI(true);
  ...
  return html`
    <div className="bg-white p-10 rounded-[3rem] shadow-2xl">
      <h2 className="text-2xl font-black mb-8">Vender Artículo Global</h2>
      <div className="space-y-5">
        <input className="w-full p-4 bg-slate-50 border-none rounded-2xl outline-none font-medium" placeholder="Título" value=${data.title} onChange=${e => setData({...data, title: e.target.value})} />
        <input className="w-full p-4 bg-slate-50 border-none rounded-2xl outline-none font-medium" placeholder="Tu Nombre" value=${data.author} onChange=${e => setData({...data, author: e.target.value})} />
        ...
        <div className="relative">
          <textarea rows="5" className="w-full p-5 bg-slate-50 border-none rounded-[2rem] outline-none resize-none leading-relaxed" placeholder="Resumen..." value=${data.content} onChange=${e => setData({...data, content: e.target.value})} />
          <button onClick=${handleGen} disabled=${loadingAI} className="absolute bottom-4 right-4 bg-indigo-600 text-white px-4 py-2 rounded-full text-[10px] font-black hover:bg-indigo-700 shadow-lg disabled:opacity-50">
            ${loadingAI ? 'CREANDO...' : '✨ IA'}
          </button>
        </div>
        <div className="flex gap-4 pt-4">
          <button onClick=${onCancel} className="flex-1 font-bold text-slate-400">Cancelar</button>
          <button onClick=${() => onSave({...data, id: Date.now().toString(), image: `${DEFAULT_IMG}?${Date.now()}`})} disabled=${isSyncing} className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-xl active:scale-95 disabled:opacity-50 transition-all">
            ${isSyncing ? 'CONECTANDO...' : 'PUBLICAR EN LA NUBE'}
          </button>
        </div>
      </div>
    </div>
